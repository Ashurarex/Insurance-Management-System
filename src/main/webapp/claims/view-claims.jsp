...
<div class="container">
    <div class="header-card">
        <div>
            ...
            <p>Register claims, view details, and update claim status.</p>
        </div>

        <a class="add-btn" href="${pageContext.request.contextPath}/claims?action=add-form">+ Add Claim</a>
    </div>

    <div class="table-card">
        <% if ("added".equals(success)) { %>
        <div class="message">Claim added successfully.</div>
        <% } else if ("approved".equals(success)) { %>
        <div class="message">Claim approved successfully.</div>
        <% } else if ("rejected".equals(success)) { %>
        <div class="message">Claim rejected successfully.</div>
        <% } else if ("pending".equals(success)) { %>
        <div class="message">Claim marked as pending.</div>
        <% } else if ("deleted".equals(success)) { %>
        <div class="message">Claim deleted successfully.</div>
        <% } %>

        <form class="search-form" action="${pageContext.request.contextPath}/claims" method="get">
            <input type="hidden" name="action" value="search">
            <input type="text" name="keyword" placeholder="Search by policy, customer, reason, or status"
                   value="<%= keyword != null ? keyword : "" %>">
            <button type="submit">Search</button>
            <a class="clear-btn" href="${pageContext.request.contextPath}/claims?action=list">Clear</a>
        </form>
        ...
        <table>
            ...
            <tr>
                ...
                <td><%= claim.getCustomerName() %></td>
                <td>₹<%= claim.getClaimAmount() %></td>
                <td><%= claim.getClaimReason() %></td>
                <td><%= claim.getClaimDate() %></td>
                <td><span class="status"><%= claim.getStatus() %></span></td>
                <td>
                    <a class="action-link approve"
                       href="${pageContext.request.contextPath}/claims?action=approve&id=<%= claim.getClaimId() %>">Approve</a>

                    <a class="action-link reject"
                       href="${pageContext.request.contextPath}/claims?action=reject&id=<%= claim.getClaimId() %>">Reject</a>

                    <a class="action-link pending"
                       href="${pageContext.request.contextPath}/claims?action=pending&id=<%= claim.getClaimId() %>">Pending</a>

                    <a class="action-link delete"
                       href="${pageContext.request.contextPath}/claims?action=delete&id=<%= claim.getClaimId() %>"
